...
- [x] Task 7: полный мутационный прогон (порог-храповик), итоговая A/B-матрица MOBILE=1 CPU=4 + десктоп GPU,
  цифры в CLAUDE.md.

## Итог
Полный мутационный прогон: 419/1175 = 35,7 % (порог 33,9 → поднят до 35,6). Матрица до/после (`OLD=b051c51`,
MOBILE=1 CPU=4, плотный поток): JS −19…−27 %, p95 ≤ 13,2 мс, q0 везде; десктоп GPU 120 fps без изменений.
Остаётся только владельцу: проверка на реальном телефоне (эмуляция CPU×4 — приближение).
